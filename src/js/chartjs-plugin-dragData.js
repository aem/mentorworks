  ...
  /******/ [
      ...
      function updateData(chartInstance, callback) {
        return function() {
          if (element && _d3Selection.event) {
            ...
            var value = chartInstance.scales[scale].getValueForPixel(
              e.clientY - chartInstance.canvas.getBoundingClientRect().top
            );
            // Allow callback to 'corece' the data if necessary
            if (typeof callback === 'function') {
              value = callback(e, datasetIndex, index, value);
            }
      ...
      var ChartJSdragDataPlugin = {
        afterInit: function afterInit(chartInstance) {
          if (chartInstance.options.dragData) {
            (0, _d3Selection.select)(chartInstance.chart.canvas).call(
              (0, _d3Drag.drag)()
                .container(chartInstance.chart.canvas)
                .on(
                  'start',
                  getElement(chartInstance, chartInstance.options.onDragStart)
                )
                .on(
                  'drag',
                  updateData(chartInstance, chartInstance.options.onDrag)
                )
                .on(
                  'end',
                  dragEndCallback(
                    chartInstance,
                    chartInstance.options.onDragEnd
                  )
                )
            );
          }
    ...
    /***/ function(module, exports, __webpack_require__) {
      ...
      (function(global, factory) {
        true
          ? factory(exports, __webpack_require__(3), __webpack_require__(4))
          ...
          function beforestart(id, container, point, that, args) {
            ...
            if (
              !d3Selection.customEvent(
                ...
                function() {
                  if (
                    (d3Selection.event.subject = s = subject.apply(
                      that,
                      args
                    )) == null
                  )
